                        ...
                        <div class="col-sm-10">
                            @if (isset($proveedores))
                                <select wire:model="proveedor" class="form-control" id="proveedor" name="proveedor">
                                    <option value="" {{ $proveedor == '' ? 'selected' : '' }}>-- Selecciona una
                                        opción --</option>
                                    @foreach ($proveedores as $proveedore)
                                        <option value="{{ $proveedore->id }}"
                                            {{ $proveedor == $proveedore->id ? 'selected' : '' }}>
                                            {{ $proveedore->nombre }}</option>
                                    @endforeach
                                </select>
                            @endif
                            @error('proveedor')
                        ...
                        <div class="mb-3 row d-flex align-items-center">
                            ...
                            <div class="col-sm-10">
                                <select name="categoria" id="categoria" wire:model="categoria_id" class="form-control"
                                    required>
                                    <option value="" {{ $categoria_id == '' ? 'selected' : '' }}>-- Selecciona
                                        Categoria --</option>
                                    @php
                                        $categoriasFinales = $tipos_producto->find($tipo_producto)->categorias;
                                    @endphp
                                    @foreach ($categoriasFinales as $categoriasMostrar)
                                        <option value="{{ $categoriasMostrar->id }}"
                                            {{ $categoria_id == $categoriasMostrar->id ? 'selected' : '' }}>
                                            {{ $categoriasMostrar->nombre }}</option>
                                    @endforeach
                                </select>
                                @error('categoria')
                                    <span class="text-danger">{{ $message }}</span>
        ...
        <div class="mb-3 row d-flex align-items-center">
            ...
            <div class="col-sm-10">
                <select wire:model="uso" class="form-control" id="uso" name="uso">
                    <option value="" {{ $uso == '' ? 'selected' : '' }}>-- Selecciona una opción --</option>
                    <option value="0" {{ $uso == '0' ? 'selected' : '' }}> Nuevo </option>
                    <option value="1" {{ $uso == '1' ? 'selected' : '' }}> Segunda mano </option>
                </select>
                @error('uso')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
        ...
        <div class="mb-3 row d-flex align-items-center">
            ...
            <div class="col-sm-10">
                <select name="resistencia_rodadura" id="resistencia_rodadura" wire:model="resistencia_rodadura"
                    class="form-control">
                    <option value="" {{ $resistencia_rodadura == '' ? 'selected' : '' }}>--</option>
                    @for ($i = 65; $i <= 71; $i++)
                        <option value="{{ chr($i) }}" {{ $resistencia_rodadura == chr($i) ? 'selected' : '' }}>
                            {{ chr($i) }}</option>
                    @endfor
                </select>
        ...
        <div class="mb-3 row d-flex align-items-center">
            ...
            <div class="col-sm-10">
                <select name="agarre_mojado" id="agarre_mojado" wire:model="agarre_mojado" class="form-control">
                    <option value="" {{ $agarre_mojado == '' ? 'selected' : '' }}>--</option>
                    @for ($i = 65; $i <= 71; $i++)
                        <option value="{{ chr($i) }}" {{ $agarre_mojado == chr($i) ? 'selected' : '' }}>
                            {{ chr($i) }}</option>
                    @endfor
                </select>
                @error('agarre_mojado')
        ...
        <div class="mb-3 row d-flex align-items-center">
            ...
            <div class="col-sm-10">
                <select name="emision_ruido" id="emision_ruido" wire:model="emision_ruido" class="form-control">
                    <option value="" {{ $emision_ruido == '' ? 'selected' : '' }}>--</option>
                    @for ($i = 67; $i <= 74; $i++)
                        <option value="{{ $i }}" {{ $emision_ruido == $i ? 'selected' : '' }}>
                            {{ $i . ' dB' }}
                        </option>
                    @endfor
                </select>
            ...
            <div class="col-sm-10">
                @if (isset($tasas))
                    <select wire:model="ecotasa" class="form-control" id="ecotasa" name="ecotasa"
                        ...
                        <option value="" {{ $ecotasa == '' ? 'selected' : '' }}>-- Selecciona una opción --
                        </option>
                        @foreach ($tasas as $tasa)
                            <option value="{{ $tasa->id }}" {{ $ecotasa == $tasa->id ? 'selected' : '' }}>
                                {{ $tasa->nombre }}</option>
                        @endforeach
                    </select>
                @endif
